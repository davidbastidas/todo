...
$(document).on('change', '.cprimarioa_select, .csecundarioa_select', function() {
	calcularValores(this);
});
function calcularValores(campo){
	...
	var cprimarioa_select=$('#'+idtr).find('.cprimarioa_select').val();
	if(cprimarioa_select=="1"){
		cprimarioa_select=3;
	}else{
		cprimarioa_select=1.73205080757;
	}
	var csecundarioa_select=$('#'+idtr).find('.csecundarioa_select').val();
	if(csecundarioa_select=="1"){
		csecundarioa_select=3;
	}else{
		csecundarioa_select=1.73205080757;
	}
	var C=(A/cprimarioa_select)/(B/csecundarioa_select);
	...
	$('#table_1 >tbody >tr').each(function() {
		if(y==1){
			json+='{'+
		        ...
		        '"cprimarioa_select": "'+$(this).find(".cprimarioa_select").val()+'",'+
		        '"csecundarioa": "'+$(this).find(".csecundarioa").val()+'",'+
		        '"csecundarioa_select": "'+$(this).find(".csecundarioa_select").val()+'",'+
		...
		for (var y = 0; y < n_filas; y++) {
			$('#table_1 tbody').append(
				'<tr id="tr_'+(y+1)+'">'+
					...
					'<td class="center input-mini"><select class="input-mini cprimarioa_select"><option value="1">/3</option><option value="2">&#8730;3</option></select></td>'+
					...
					'<td class="center input-mini"><select class="input-mini csecundarioa_select"><option value="1">/3</option><option value="2">&#8730;3</option></select></td>'+
			...
			$('#table_1 tbody #tr_'+(y+1)).find('.cprimarioa_select').val(obj.tabla[y].cprimarioa_select);
			$('#table_1 tbody #tr_'+(y+1)).find('.csecundarioa_select').val(obj.tabla[y].csecundarioa_select);
...
<div class="row-fluid">
  <div class="span6">
	<h3>Relacion de Transformacion TP's</h3>
  </div>
  <div class="span6">
	...
	<table id="table_1" class="table table-bordered">
		<thead>
			<tr>
				...
				<th class="center" colspan="2">Tension Primario (V)</th>
				<th class="center" colspan="2">Tension Secundario (V)</th>
				<th class="center">Relacion Calculada</th>
				<th class="center">Tension Inyectada (V))</th>
				<th class="center">Tension Medida (V)</th>
