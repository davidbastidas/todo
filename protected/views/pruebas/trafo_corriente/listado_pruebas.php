...
'<div class="breadcrumbs fixed" id="breadcrumbs">'.
	'<ul class="breadcrumb">'.
		...
		'</li>'.
		'<li>'.
			'Lista de Pruebas Transformador de Corriente'.
		'</li>'.
	'</ul>'.
'</div>';
...
<script src="<?php  echo Yii::app() -> theme -> baseUrl . '/views'; ?>/js/fuelux/fuelux.spinner.min.js"></script>
<script>
	...
	function validarDevanadoMostrarTemp(){
		...
		if(text.length>0){
			...
			$('#temp-ambiente').ace_spinner({value:parseInt(obj.temperaturas[0].temp_ambiente),min:0,max:50,step:5, icon_up:'icon-plus', icon_down:'icon-minus', btn_up_class:'btn-success' , btn_down_class:'btn-danger'});
			$('#temp-aceite').ace_spinner({value:parseInt(obj.temperaturas[0].temp_aceite),min:0,max:50,step:5, icon_up:'icon-plus', icon_down:'icon-minus', btn_up_class:'btn-success' , btn_down_class:'btn-danger'});
		}else{
			$('#temp-ambiente').ace_spinner({value:0,min:0,max:50,step:5, icon_up:'icon-plus', icon_down:'icon-minus', btn_up_class:'btn-success' , btn_down_class:'btn-danger'});
			$('#temp-aceite').ace_spinner({value:0,min:0,max:50,step:5, icon_up:'icon-plus', icon_down:'icon-minus', btn_up_class:'btn-success' , btn_down_class:'btn-danger'});
		}
		if (estado==3){
...
<div class="row-fluid">
	<h2>Transformador de Corriente</h2>
	<div class="span12 form-inline">
		<h4>Por favor elija la temperatura proxima</h4>
		<?php 
			$eqjson = json_decode( $equipojson, true );
			if(isset($eqjson['tipo_mecanismo'])){
				if($eqjson['tipo_mecanismo']=="Solido"){?>
				<label for="temp-ambiente">Temp. Ambiente</label>
				<input type="text" class="input-mini" id="temp-ambiente" disabled/><label style="font-size:20px;">&#176;C</label>
		<?php 
				}else if($eqjson['tipo_mecanismo']=="Aceite"){
		?>
				<label for="temp-aceite">Temp. Ambiente</label>
				<input type="text" class="input-mini" id="temp-aceite" disabled/><label style="font-size:20px;">&#176;C</label>
		<?php 
				}
			}else{
				echo "Faltan guardar el equipo como Tipo Solido o Aceite. Consulte con el Administrador.";
			}
		?>
		<br><br>
