...
$(document).on("keyup", ".capacitancia", function(){
	...
	/*var C1=$('#table_1 #'+idtr).find('.capacitancia').val();
	var C2=$('#table_2 #'+idtr).find('.capacitancia').val();
	var DIF=Math.abs(C1-C2);
	var PORC=(DIF/C1)*100;
	if(PORC<=5){
		$('#table_1 #'+idtr).find('.capacitancia').css({
		   'text-decoration': 'underline',
		   'color': 'green'
		});
		$('#table_2 #'+idtr).find('.capacitancia').css({
		   'text-decoration': 'underline',
		   'color': 'green'
		});
	}else{
