...
Yii::import('DDataColumn');
Yii::import('DLinkColumn');
Yii::import('DButtonColumn');
Yii::import('DCheckBoxColumn');
 ...
 * @since 1.1
 */
class DGridView extends CBaseListView
{
	 ...
	 * where "type" and "header" are optional. A {@link DDataColumn} instance will be created in this case,
	 * whose {@link DDataColumn::name}, {@link DDataColumn::type} and {@link DDataColumn::header}
	 ...
	 * the 'class' element specifies the column class name (defaults to {@link DDataColumn} if absent).
	 * Currently, these official column classes are provided: {@link DDataColumn},
	 ...
	 * This differs from {@link nullDisplay} in that {@link nullDisplay} is only used by {@link DDataColumn} to render
	 * null data values.
	 * @since 1.1.7
	 */
	public $blankDisplay='&nbsp;';
	 ...
	 * @since 1.1.1
	 */
	public $filterCssClass='filters';//filters
	/**
	 ...
	 * Note that in order to show an input field for filtering, a column must have its {@link DDataColumn::name}
	 * property set or have {@link DDataColumn::filter} as the HTML code for the input field.
	...
	/**
	 * Creates a {@link DDataColumn} based on a shortcut column specification string.
	 * @param string $text the column specification string
	 * @return DDataColumn the column instance
	 */
	protected function createDataColumn($text)
	{
		if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$text,$matches))
			throw new CException(Yii::t('zii','The column must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
		$column=new DDataColumn($this);
